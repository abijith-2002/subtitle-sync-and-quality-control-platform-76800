...
def process_subtitle_overlap(
    ...
    # --- Patch header and style for robust ASS output: match the known valid .ass file ---

    # [Script Info]
    # Example from output1.ass and standard for Aegisub:
    # ; Script generated by pysubs2
    # Title: (usually blank or program name)
    # ScriptType: v4.00+
    # Collisions: Normal
    # PlayResX: ...
    # PlayResY: ...
    ...
    # Ensure keys in correct order for maximum compatibility
    subs.info["Title"] = ""
    ...
    # Remove pysubs2 "YCbCr Matrix" or others that are unneeded in this context
    # (pysubs2 may add more fields, that's OK as long as the above are set and emitted in order)

    # [V4+ Styles]: match layout/definitions from user output1.ass
    from pysubs2 import SSAStyle

    # User's file uses (vivid) example:
    # Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding
    # Style: Default,Arial,20,&H00FFFFFF,&H000000FF,&H00000000,&H00000000,0,0,0,0,100,100,0,0,1,2,2,2,10,10,10,1

    # Compose color values as in example
    style_params = dict(
        fontname="Arial",
        fontsize=20,
        ...
        outline=2,
        shadow=2,
        alignment=2,  # ASS alignment for Default style (bottom center)
        marginl=10,
        marginr=10,
        marginv=20,   # MarginV = 20 as required
        encoding=1
    )
    ...
    style_default = SSAStyle(
        ...
        secondarycolor=style_params["secondarycolor"],
        outlinecolor=style_params["outlinecolor"],
        backcolor=style_params["backcolor"],
        bold=-1 if style_params["bold"] else 0,
        italic=-1 if style_params["italic"] else 0,
        underline=-1 if style_params["underline"] else 0,
        strikeout=-1 if style_params["strikeout"] else 0,
        scalex=style_params["scalex"],
        scaley=style_params["scaley"],
        spacing=style_params["spacing"],
        angle=style_params["angle"],
        borderstyle=style_params["borderstyle"],
        outline=style_params["outline"],
        shadow=style_params["shadow"],
    ...
    for event in subs.events:
        if getattr(event, "style", None) is None or event.style == "":
            event.style = "Default"
        # Ensure MarginL, MarginR, MarginV match the style (10, 10, 20)
        event.marginl = 10
        event.marginr = 10
        event.marginv = 20
        # ASS field order: (Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text)
        # pysubs2 already emits this but we forcibly set the fields

    try:
        # Save with strict format - force ASS format (pysubs2 is standards compliant)
